          ...
          <el-table-column label="操作"
            ...
            <template slot-scope="scope">
              <el-button type="success"
                         @click="setStatus(scope.row, 1)">已处理</el-button>
              <el-button type="danger"
                         @click="setStatus(scope.row, 2)">待审核</el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    setStatus(row, type) {
      ...
      this.$axios.postFormData(this.API.SET_STATUS_API, param)
        .then((res) => {
          if (res.success === true) {
            this.$message.success(res.message || '设置成功！');
            this.tableDataList(this.getParams());
          } else {
            this.$message.error(res.message || '设置失败，请重试！');
          }
        });
    }
  }
