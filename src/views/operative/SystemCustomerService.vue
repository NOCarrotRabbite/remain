    ...
    <el-dialog title="新增客服"
      ...
      <span slot="footer"
        ...
        <el-button type="primary"
                   @click="addService">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    tableDataList() {
      ...
      this.$axios.postFormData(this.API.CUSTOMER_API, param)
        .then((res) => {
          if (res.success == true) {
    ...
    addService() {
      ...
      this.$axios.postFormData(this.API.ADD_CUSTOMER_API, param)
        .then((res) => {
          if (res.success == true) {
    ...
    delUser(data) {
      this.$confirm('您确定删除此客服吗?', '提示', {
        ...
        this.$axios.postFormData(this.API.DELETE_CUSTOMER_API, param)
          .then((res) => {
            if (res.success == true) {
